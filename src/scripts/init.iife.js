(function (event, webviewWindow) {
    ...
    async function handleIframeRpcRequest(event$1) {
        console.log('TAURI-PLUGIN-MCP: Received iframe-rpc, payload:', event$1.payload);
        try {
            const { method, args } = event$1.payload;
            const iframe = document.getElementById('moss-preview-iframe');
            if (!iframe?.contentWindow) {
                await event.emit('iframe-rpc-response', { success: false, error: 'No iframe found' });
                return;
            }
            ...
            await event.emit('iframe-rpc-response', response);
            ...
            await event.emit('iframe-rpc-response', {
                success: false,
                error: error instanceof Error ? error.toString() : String(error)
            }).catch(e => console.error('TAURI-PLUGIN-MCP: Error emitting error response', e));
        }
...
})(__TAURI__.event, __TAURI__.webviewWindow);
